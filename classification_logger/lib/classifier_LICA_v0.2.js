...
let payload_files = {
  "keywords": "payload_lica.json",
  "stopwords": "stopwords.json",
  "mozcat_heirarchy": "mozcat_heirarchy.json",
  "domain_rules": "payload_domain_rules.json"
}
//auxiliary functionality
//functions are wrapped into an object so they're not hanging around in global space
  ...
  isSingleTopicPath: function(parsedURL){
    ...
    if (payload_files.path_rules.hasOwnProperty(parsedURL.tld)) {
      if (parsedURL.path.length > 0) {
        ...
        if (payload_files.path_rules[parsedURL.tld].hasOwnProperty(first_chunk)) { //note that this currently only checks
          let tmpResult = payload_files.path_rules[parsedURL.tld][first_chunk]; //1 level of path i.e. these are the same:
          tmpResult.push(['single_topic_path']); //domain.com/tech and domain.com/tech/apple
          return tmpResult;
        }
      }
    }
...
function LICA(){
    ...
    if (matching.containsStopwords(words, 'web')) {//check that there are no ignored web words like "login" (don't want to catch some
      return ['uncategorized', 'ignored', 'ignored_words']; //accidentally unencrypted personal data)
    }
